<script lang="ts">
	...
	const eventSchema = $derived(
		generateEventSchema({
			name: buildEventName('Transport, Energy and Climate Economics (TrEnCE)', seminar.title),
			description: seoDescription,
			startDate: seminar.date,
			location: seminar.location ? { name: seminar.location } : undefined,
			presenter: seminar.presenter,
			url: `/seminaires/trence/${seminar.slug}`
		})
	);
	...
	const breadcrumbSchema = $derived(
		generateBreadcrumbSchema([
			...
			{ name: 'Transport, Energy and Climate Economics (TrEnCE)', url: '/seminaires/trence' },
			{ name: seminar.title, url: `/seminaires/trence/${seminar.slug}` }
		])
	);
</script>

<SEO
	title="TrEnCE: {seminar.title} | Institut ACSS-PSL"
	description={seoDescription}
	type="article"
/>
...
<article class="seminar-detail">
	...
	<!-- Workshop Info Card -->
	<WorkshopInfo type="TrEnCE" variant="section" showRegistration={!isPast} />

	<!-- Back link -->
	<div class="back-link">
		<a href="/seminaires/trence">
			<svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
				<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
			</svg>
			Back to all TrEnCE sessions
		</a>
	</div>
</article>
